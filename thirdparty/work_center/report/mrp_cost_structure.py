...
class MrpCostStructure(models.AbstractModel):
    ...
    def get_lines(self, productions):
        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                query_str = """SELECT w.operation_id, op.name, partner.name, sum(t.duration), wc.costs_hour,wc.other_cost
                                ...
                                GROUP BY w.operation_id, op.name, partner.name, t.user_id, wc.costs_hour,wc.other_cost
                                ORDER BY op.name, partner.name
                            """
                self.env.cr.execute(query_str, (tuple(Workorders.ids),))
                for op_id, op_name, user, duration, cost_hour, other_cost in self.env.cr.fetchall():
                    operations.append(
                        [user, op_id, op_name, duration / 60.0, cost_hour,other_cost])
            ...
            scraps = StockMove.search(
                [('production_id', 'in', mos.ids), ('scrapped', '=', True),
                 ('state', '=', 'done')])
            ...
            if not all(m.product_uom_id.id == uom.id for m in mos):
                uom = product.uom_id
                for m in mos:
                    qty = sum(m.move_finished_ids.filtered(lambda
                                                               mo: mo.state != 'cancel' and mo.product_id == product).mapped(
                        'product_qty'))
                ...
                for m in mos:
                    mo_qty += sum(m.move_finished_ids.filtered(lambda
                                                                   mo: mo.state != 'cancel' and mo.product_id == product).mapped(
                        'product_qty'))
            for m in mos:
                byproduct_moves = m.move_finished_ids.filtered(lambda
                                                                   mo: mo.state != 'cancel' and mo.product_id != product)
            res.append({
                'product': product,
                'mo_qty': mo_qty,
                'mo_uom': uom,
