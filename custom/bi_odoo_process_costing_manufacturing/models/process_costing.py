...
class MrpBom(models.Model):
    ...
    bom_material_cost_ids = fields.One2many("mrp.bom.material.cost",
                                            "mrp_bom_material_id",
                                            "Material Cost")
    bom_labour_cost_ids = fields.One2many("mrp.bom.labour.cost",
                                          "mrp_bom_labour_id", "Labour Cost")
    bom_overhead_cost_ids = fields.One2many("mrp.bom.overhead.cost",
                                            "mrp_bom_overhead_id",
                                            "Overhead Cost")
    # single page total cost
    bom_total_material_cost = fields.Float(compute='_compute_total_cost',
                                           string="Total Material Cost",
                                           default=0.0)
    bom_total_labour_cost = fields.Float(compute='_compute_total_cost',
                                         string="Total Labour Cost",
                                         default=0.0)
    bom_total_overhead_cost = fields.Float(compute='_compute_total_cost',
                                           string="Total Overhead Cost",
                                           default=0.0)
    currency_id = fields.Many2one("res.currency", compute='get_currency_id',
                                  string="Currency")
        ...
        if vals.get('product_id'):
            ...
            material_product_id = self.env['mrp.bom.material.cost'].search(
                [('product_id', '=', old_product.id),
                 ('mrp_bom_material_id', '=', self.bom_id.id)])
            material_vals = {
                'product_id': product_id.id,
...
class MrpBomMaterialCost(models.Model):
    ...
    total_cost = fields.Float(compute='onchange_planned_qty',
                              string="Total Cost")
    total_actual_cost = fields.Float(compute='onchange_planned_qty',
                                     string="Total Actual Cost")
    mrp_bom_material_id = fields.Many2one("mrp.bom", "Mrp Bom Material")
    mrp_pro_material_id = fields.Many2one("mrp.production",
                                          "Mrp Production Material")
    mrp_wo_material_id = fields.Many2one("mrp.workorder",
                                         "Mrp Workorder Material")
    currency_id = fields.Many2one("res.currency", compute='get_currency_id',
                                  string="Currency")
...
class MrpBomLabourCost(models.Model):
    ...
    uom_id = fields.Many2one('uom.uom', string="UOM")
    cost = fields.Float(string="Cost/Unit")
    total_cost = fields.Float(compute='onchange_labour_planned_qty',
                              string="Total Cost")
    total_actual_cost = fields.Float(compute='onchange_labour_planned_qty',
                                     string="Total Actual Cost")
    # total_labour_cost = fields.Float(string="Total Labour Cost")
    mrp_bom_labour_id = fields.Many2one("mrp.bom", "Mrp Bom Labour")
    mrp_pro_labour_id = fields.Many2one("mrp.production",
                                        "Mrp Production Labour")
...
class MrpBomOverheadCost(models.Model):
    ...
    uom_id = fields.Many2one('uom.uom', string="UOM")
    cost = fields.Float(string="Cost/Unit")
    total_cost = fields.Float(compute='onchange_overhead_planned_qty',
                              string="Total Cost")
    total_actual_cost = fields.Float(compute='onchange_overhead_planned_qty',
                                     string="Total Actual Cost")
    # total_overhead_cost = fields.Float(string="Total Overhead Cost")
    mrp_bom_overhead_id = fields.Many2one("mrp.bom", "Mrp Bom Overhead")
    mrp_pro_overhead_id = fields.Many2one("mrp.production",
                                          "Mrp Production Overhead")
    mrp_wo_overhead_id = fields.Many2one("mrp.workorder",
                                         "Mrp Workorder Overhead")
    currency_id = fields.Many2one("res.currency", compute='get_currency_id',
                                  string="Currency")
...
class MrpProduction(models.Model):
    ...
    def _generate_workorders(self, exploded_boms):
        ...
        for bom, bom_data in exploded_boms:
            ...
            if bom.routing_id.id and (not bom_data['parent_line'] or bom_data[
                'parent_line'].bom_id.routing_id.id != bom.routing_id.id):
                workorders += self._workorders_create(bom, bom_data)
                workorders.write(
                    {'wo_material_cost_ids': [(6, 0, list_of_material)],
    ...
    def create(self, vals):
        ...
        for material in res.move_raw_ids:
            ...
            material_obj = self.env['mrp.bom.material.cost'].create(vals)
            material_obj.write({
                "total_actual_cost": material_obj.actual_qty * material_obj.cost })
    ...
    pro_material_cost_ids = fields.One2many("mrp.bom.material.cost",
                                            "mrp_pro_material_id",
                                            "Material Cost")
    pro_labour_cost_ids = fields.One2many("mrp.bom.labour.cost",
                                          "mrp_pro_labour_id", "Labour Cost")
    pro_overhead_cost_ids = fields.One2many("mrp.bom.overhead.cost",
                                            "mrp_pro_overhead_id",
                                            "Overhead Cost")
    pro_total_material_cost = fields.Float(string="Total Material Cost",
                                           default=0.0)

    # Costing Tab
    total_material_cost = fields.Float(compute='_compute_total_cost',
                                       string="Total Material Cost",
                                       default=0.0)
    total_labour_cost = fields.Float(compute='_compute_total_cost',
                                     string="Total Labour Cost", default=0.0)
    total_overhead_cost = fields.Float(compute='_compute_total_cost',
                                       string="Total Overhead Cost",
                                       default=0.0)
    total_all_cost = fields.Float(compute='_compute_total_all_cost',
                                  string="Total Cost", default=0.0)

    # Costing Tab
    total_actual_material_cost = fields.Float(compute='_compute_total_cost',
                                              string="Total Actual Material Cost",
                                              default=0.0)
    total_actual_labour_cost = fields.Float(compute='_compute_total_cost',
                                            string="Total Actual Labour Cost",
                                            default=0.0)
    total_actual_overhead_cost = fields.Float(compute='_compute_total_cost',
                                              string="Total Actual Overhead Cost",
                                              default=0.0)
    total_actual_all_cost = fields.Float(compute='_compute_total_all_cost',
                                         string="Total Actual Cost",
                                         default=0.0)

    product_unit_cost = fields.Float(compute='_compute_total_product_cost',
                                     string="Product Unit Cost", default=0.0)
...
class MrpWorkorder(models.Model):
    ...
    wo_material_cost_ids = fields.One2many("mrp.bom.material.cost",
                                           "mrp_wo_material_id",
                                           "Material Cost")
    wo_labour_cost_ids = fields.One2many("mrp.bom.labour.cost",
                                         "mrp_wo_labour_id", "Labour Cost")
    wo_overhead_cost_ids = fields.One2many("mrp.bom.overhead.cost",
                                           "mrp_wo_overhead_id",
                                           "Overhead Cost")
    ...
    def change_prod_qty(self):
        ...
        for wizard in self:
            ...
            if wizard.product_qty < produced:
                ...
                raise UserError(_(
                    "You have already processed %s. Please input a quantity higher than %s ") % (
                                    format_qty % produced,
                                    format_qty % produced))
            ...
            for material in production.pro_material_cost_ids:
                for bom in production.bom_id.bom_line_ids:
                    if material.product_id.id == bom.product_id.id:
                        material.write({
                            'planned_qty': wizard.product_qty * bom.product_qty,
            ...
            for labour in production.pro_labour_cost_ids:
                for bom in production.bom_id.bom_labour_cost_ids:
                    if labour.product_id.id == bom.product_id.id:
                        labour.write({
                            'planned_qty': wizard.product_qty * bom.planned_qty,
            ...
            done_moves = production.move_finished_ids.filtered(lambda
                                                                   x: x.state == 'done' and x.product_id == production.product_id)
            ...
            boms, lines = production.bom_id.explode(production.product_id,
                                                    factor,
                                                    picking_type=production.bom_id.picking_type_id)
            documents = {}
            for line, line_data in lines:
                move, old_qty, new_qty = production._update_raw_move(line,
                                                                     line_data)
            ...
            for wo in production.workorder_ids:
                ...
                if operation_bom_qty.get(operation.id):
                    cycle_number = float_round(operation_bom_qty[
                                                   operation.id] / operation.workcenter_id.capacity,
                                               precision_digits=0,
                                               rounding_method='UP')
                ...
                if production.product_id.tracking == 'serial':
                    quantity = 1.0 if not float_is_zero(quantity,
                                                        precision_digits=precision) else 0.0
                else:
                ...
                moves_raw = production.move_raw_ids.filtered(lambda
                                                                 move: move.operation_id == operation and move.state not in (
                    'done', 'cancel'))
                ...
                if quantity > 0 and wo.move_raw_ids.filtered(lambda
                                                                     x: x.product_id.tracking != 'none') and not wo.active_move_line_ids:
